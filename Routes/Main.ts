...
import { ISign, SignModel } from '../Schema/Post.js';
...
router.get('/changepass',(req,res)=>{
  ...
  router.post('/changepass', async (req: Request, res: Response) => {
    ...
    const user = res.locals.user as ISign;
    if (!user) {
    ...
    try {
      ...
      const isMatch = await bcrypt.compare(current_password, user.Password as string);
      if (!isMatch) {
  ...
  router.get('/candmlist', (req: Request, res: Response) => {
    res.sendFile(path.resolve(__dirname, '../Views/createList.html'));
});
router.post('/createList',async(req:Request,res:Response)=>{
